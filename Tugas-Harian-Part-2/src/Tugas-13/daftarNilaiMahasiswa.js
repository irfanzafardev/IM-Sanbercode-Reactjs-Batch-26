...
const NilaiMahasiswa = () => {
	...
	const handleEdit = async (event) => {
		...
		const result = await axios.get(
			`http://backendexample.sanbercloud.com/api/student-scores/${editedId}`
		);
	...
	const handleDelete = (event) => {
		...
		axios
			.delete(
				`http://backendexample.sanbercloud.com/api/student-scores/${deletedId}`
			)
			.then(() => {
				// clear current id checking
				let conditionalCurrentId =
					input.currentId === deletedId ? { currentId: null } : {};
